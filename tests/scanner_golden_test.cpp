...
// Result of running the real binary with --run-stage scanner.
struct RunResult {
  int exit_code = -1;
  std::string out;
  std::string err;
...
RunResult RunScanner(llvm::StringRef rel_path) {
  ...
  llvm::SmallVector<llvm::StringRef, 4> args{kBinary, "--run-stage", "scanner", invocation};
...
// the scanner reported lexical errors (exit 1), an empty one a clean stream
...
class ScannerGoldenTest : public testing::TestWithParam<std::string> {};

TEST_P(ScannerGoldenTest, ScannerOutputMatchesGolden) {
  const std::string &rel = GetParam();
  const RunResult result = RunScanner(rel);
